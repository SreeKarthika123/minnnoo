...
// // ANALYZE ALL VACANCIES FOR USER
// router.post("/analyze-all/:userId", async (req, res) => {
//   try {
//     const { userId } = req.params;
//     const vacancies = await Vacancy.find();
...
//         {
//           $push: {
//             preScores: { userId, score: preScore, vector }
//           }
//         }
//       );
...
//     // ===== PHASE 2: TOP MATCHES USING COSINE SIMILARITY =====
//     const updatedVacancies = await Vacancy.find();
//     const topMatches = [];
...
//       v.preScores?.forEach(p => {
//         if (p.userId.toString() === userId && Object.keys(p.vector || {}).length > 0) {
//           const similarity = Math.round(cosineSimilarity(p.vector, jobVector) * 100);
//           topMatches.push({
//             vacancyId: v._id,
//             title: v.title,
//             company: v.company,
...
//     res.json({
//       message: "Analysis completed using cosine similarity",
//       topMatches: top5
//     });
//   } catch (err) {
//     console.error("Auto analysis failed:", err);
//     analysisProgress[req.params.userId].status = "failed";
//     res.status(500).json({ message: "Auto analysis failed" });
//   }
// });
...
// // GET DASHBOARD MATCHES (USER TOP JOBS)
// router.get("/dashboard-matches/:userId", async (req, res) => {
//   const { userId } = req.params;
//   const vacancies = await Vacancy.find();
...
//   vacancies.forEach(v => {
//     v.preScores?.forEach(p => {
//       if (p.userId.toString() === userId && p.score > 0) {
//         matches.push({
//           vacancyId: v._id,
//           title: v.title,
//           company: v.company,
...
// // Compute score for a single vacancy
// router.post("/match-score", async (req, res) => {
//   try {
//     const { userId, vacancyId } = req.body;
...
//     const { score } = await computePreScore(userId, vacancy);

//     // Save the score
//     await Vacancy.updateOne(
//       { _id: vacancyId },
//       { $pull: { preScores: { userId } } } // remove old
//     );
//     await Vacancy.updateOne(
//       { _id: vacancyId },
//       { $push: { preScores: { userId, score, vector: {} } } }
//     );

//     res.json({ score, summary: "Computed via cosine similarity" });
//   } catch (err) {
//     console.error(err);
//     res.status(500).json({ message: "Score computation failed" });
//   }
// });
